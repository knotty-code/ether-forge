...
from plumber.api.v1alpha1.pysrc.linkappstate import LINKAPPSTATE_SCHEMA
from plumber.intents.linkapp.handlers import get_config_handler
from plumber.intents.linkapp.init import init_globals_defaults, validate
from plumber.intents.linkapp.agent import LinkAppAgent
def process_cr(cr):
    log_msg("CR:", dict=cr)
    log_msg("CR received. Handing off to the Agent...")
    cr_obj = LinkApp.from_input(cr)
    if cr_obj is None:
